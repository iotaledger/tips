...
it respects the dependency of bundles, ensuring that the `parent1` and `parent2` of a bundle are applied before it. Since
...
starting from a milestone and by going first through `parent1` bundle, then `parent2` bundle and finally current bundles.
