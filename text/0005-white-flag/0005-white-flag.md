...
In case of conflicting bundles with White Flag, nodes will confirm only one bundle and ignore all the others. For this
to work, they need to be sure they are all confirming the same bundle; hence, the need for a deterministic ordering
of the tangle.
...
The deterministic ordering of the past-cone of a milestone is a [topological ordering](https://en.wikipedia.org/wiki/Topological_sorting)
generated by a post-order Depth-First Search by going first through trunks, then branches and finally root.
The stopping condition of this traversal is reaching bundles that are already confirmed.

This ordering doesn't come with any extra overhead because it can also be used to actually compute the ledger state at
the same time.

![][tangle]

For example: the topological ordering of the set of bundles referenced by the milestone `V` (purple set) is then
`{D, G, J, L, M, R, I, K, N, O, S, V}`. If a conflict was occurring in this set, nodes would confirm the first of the
conflicting bundles and ignore the others.
