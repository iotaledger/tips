...
Solving the DLT scalability dilemma ultimatively leads to lower transaction costs. Therefore other DLT projects will also run into the same dust obstacle in the end. IOTA solves this problem with the `storage deposit`.
...
This means that there is no risk for Alice to loose the `storage deposit`, because either Bob needs to return the specified `Return Amount`, or the ownership of the created output switches back to Alice after the specified time-window has expired.
