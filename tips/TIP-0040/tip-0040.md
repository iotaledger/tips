...
Stakers, or interchangeably called _registered validators_, lock their IOTA tokens to _stake_ them. By locking the
tokens, the stakers prove control over them so the protocol assigns them a proportional amount of stake in exchange.
Additionally, other users - so-called delegators - can _delegate_ their IOTA tokens to a staker to increase their stake
...
determine the ledger state. Stake is only used to determine who is selected for the committee. Once selected, every
