...
- For any validator `i` in the set of _registered validators_ the voting power is equal to `Stake_i + DelegatedStake_i`
  ...
  - `DelegatedStake_i` is the sum of all _Delegation Output's_ `Amount` field where the value of the `Validator ID` field
    is equal to `i`.
...
with `Validator ID`s as keys, of type `ByteArray[32]`, and the values being a list of objects consisting of
these fields:
...
The rewards of an epoch are tracked by keeping a map with `Validator ID`s as keys, of type `ByteArray[32]`, and
...
- Let the `Claimable Rewards` be all reward entries for the validator identified by the Account ID `Validator ID` field in
