...
By locking the tokens, the stakers prove control over them, so the protocol assigns them a proportional amount of voting power in exchange. 
Other users - the so-called delegators - can _delegate_ their IOTA tokens to a staker to further increase their voting power. 
...
Only the voting power of those selected into the committee is considered to determine the _Witness Weight_, used for the acceptance of blocks, 
