...
- Check if the total number of validation blocks issued in this slot is larger than `TargetValidationBlocks`.
  ...
  In the case that `TargetValidationBlocks` is larger or equal than `2^8-1`, this "tricky" can overflow the variables used.
...
- Given the values defined above, then `Profit Margin = (Total Validator Stake<<ProfitMarginExponent)/(Total Validator Stake + Total Stake)`. Note that this will be an integer between 0 and `2^ProfitMarginExponent`
