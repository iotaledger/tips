...
The weight that validators give to blocks is derived from the total stake of the pool (i.e., from the locked tokens of the staker and the delegated tokens of all its delegators). 
...
TODO: Additionally, motivate using seat-based committees where every seat has the same weight rather than using the weight
...
<table>
  ...
  <tr>
    ...
    <td>uint64</td>
    <td>The amount of delegated IOTA tokens.</td>
  </tr>
  <tr>
    <td>Delegation ID</td>
    <td>ByteArray[32]</td>
...
- `Decay Balancing Constant Coefficient` = ==TO DO: choose a value, this is arbitrary, just make sure it does not
  overflow==
- `Decay Balancing Constant = ` (integer approximation of
  ...
  duration in years, $T=$ `Bootstrapping Duration`, and $\beta=1/3$). ==TO DO: discuss about approximating this to e==
...
==to do: re check if this works for any number of bits==
