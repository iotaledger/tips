...
- Let the `Claimable Rewards` be all reward entries for the validator identified by the Account ID `Validator ID` field
  ...
  where:
  - If the output is in _Delegating State_ let `Delegation End` be `Current Transaction Epoch Index - 1`.
...
Mana Rewards for some epoch `X` only become available after that epoch ends. At that point, the voting power for the
...
- Delegation Output X is created before the end of the Registration Slot. Therefore its `Start Epoch` is set to `21`, as
  ...
  never included in any voting power calculation.
- Delegation Output Y is created before the end of the Registration Slot. Therefore its `Start Epoch` is set to `21`, as
  ...
  Slot, so it was included in the voting power calculation. Therefore the last epoch for which it could get rewards is
...
- Delegation Output Z is analogous to X, only shifted by one epoch. It is created after the Registration Slot of Epoch
  ...
  will not be included in the voting power calculation for said epoch. Its `End Epoch` is thus set to `21`, as that may
  be the last epoch for which it could get rewards. Simlar to Output X, the delegation output will not be able to claim
  any rewards as it was never included in any voting power calculation and no epoch index `A` satisfies
  `A >= 22 && A <= 21`.
