...
- Let the `Claimable Rewards` be all reward entries for the validator identified by the Account ID `Validator` field in the output, where its `Epoch Index` satisfies: `Epoch Index >= Start Epoch && Epoch Index < Delegation End`, where:
