...
accounts can issue blocks by signing blocks with the private key corresponding to the public key from which the
_Implicit Account Creation Address_ was derived.
...
- Let `Min Block Issuer Account Amount` be the amount of IOTA coins required to cover the storage deposit for a minimal
  Account Output that contains a Block Issuer Feature. To calculate this amount, the storage deposit of an account with
  all required fields set is taken and which fulfills the following conditions:
