...
- The following conditions must all hold:
  ...
  - Each key in `Block Issuer Keys` must be unique based on its `Public Key`. No duplicates are allowed.
  - Each key in `Block Issuer Keys` must be lexicographically ordered based on its `Public Key`.
