...
- If a transaction contains an input with an unlock condition containing an implicit account address, the transaction is
  ...
  - An `Account` Output is created on the output side, for which all of the following conditions hold:
    - TODO: Fix confusion between implicit account id mentioned here and further above in the account TX section.
    <!-- - The Account Output must be valid according to   which must be a valid account creation transaction :
    - The `Account ID` of the `Account` must be set to the `Implicit Account ID`.
    - The `Account` has a Block Issuer Feature. -->
