...
- The storage deposit of an _Implicit Account Creation Address_ defines a dedicated VByte Offset. It is equal to the
  VBytes required for a minimal Account Output that contains a Block Issuer Feature. To calculate this amount, the
  VBytes of an account with all required fields set is taken and which fulfills the following conditions:
