...
- An output that has a <i>Manalock Unlock Condition</i> specified must only be consumed and unlocked in a
  ...
  condition.
    - the specified `Account Address` of the <i>Address Unlock Condition</i> has a non-negative BIC balance at `Transaction Slot Index - MCA` where `Transaction Slot Index` is the index of the slot to which the transaction belongs.
...
<!-- 
- When a <i>Block Issuer Feature</i> is present in an account input containing `Expiry Slot >= Slot Index` or in an account output representing the initial state of its UTXO state machine, the transaction that contains the account output is valid, if either of the following conditions for the account output hold:
    - `Expiry Slot` = `0`.
    - `Expiry Slot` >= `Slot Index + MCA`, where the `Slot Index` is the index of the slot to which the transaction belongs and `MCA` is the maximum committable age (TODO: Define or link to definition) -->
