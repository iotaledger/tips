...
Note: This address type has additional transaction validation rules specified in [TIP-42](../TIP-0042/tip-0042.md) as
well as in [Implicit Account Creation Address Capabilities](#implicit-account-creation-address-capabilities).
...
of the bit. This list is an Allowlist, meaning it allows capabilities when the bit is `1` and disallows it if the bit is
`0`. This table defines the disallowed capabilities, as the transaction validation is only affected if the flag is
**unset**.

| Flag Index | Disallowed Capability (if flag is unset)                             |
| ---------- | -------------------------------------------------------------------- |
| 0          | Cannot receive Native Tokens.                                        |
| 1          | Cannot receive Mana.                                                 |
| 2          | Cannot receive Outputs with Timelock Unlock Condition.               |
| 3          | Cannot receive Outputs with Expiration Unlock Condition.             |
| 4          | Cannot receive Outputs with Storage Deposit Return Unlock Condition. |
| 5          | Cannot receive Account Outputs.                                      |
| 6          | Cannot receive NFT Outputs.                                          |
| 7          | Cannot receive Delegation Outputs.                                   |
...
defined in terms of the flags being **unset** (bit == `0`). If the flag is **set** (bit == `1`), the rule does not apply
to the transaction.
...
#### Implicit Account Creation Address Capabilities

An Implicit Account Creation Address has hardcoded capability flags instead of configurable ones. These flags are not
serialized in the address but still apply to transaction validation. The following flags are **set**: `1`. Other flags
are **unset**.
