...
The [Output Design](#output-design) section lists all supported <i>Unlock Conditions</i> and <i>Featue Blocks</i> for
each output type.
...
Transaction validation rules are detailed in [draft TIP-18](https://github.com/iotaledger/tips/pull/40).
...
This unlock condition is employed to achieve conditional sending. An output that <i>Dust Deposit Unlock Condition</i>
specified can only be consumed in a transaction that deposits `Return Amount` IOTA coins into `Sender` address. When
several of such outputs are consumed, their return amounts per `Sender` addresses are summed up and the output side
must deposit this total sum per `Sender` address.

###### Additional syntactic transaction validation rule:

- `Return Amount` must be ≥ than `Minimum Dust Deposit`.
- `Return Amount` in a <i>Dust Deposit Unlock Condition</i> must be ≤ than the required dust deposit of the output.

###### Additional semantic transaction validation rule:

- An output that has <i>Dust Deposit Unlock Condition</i> specified must only be consumed and unlocked in a transaction
  that deposits `Return Amount` IOTA coins to `Sender` address via an output that has no additional spending
  constraints. (<i>ExtendedOutput</i> without feature blocks)
- When several outputs with <i>Dust Deposit Unlock Condition</i> and the same `Sender` are consumed, their return
  amounts per `Sender` addresses are summed up and the output side of the transaction must deposit this total sum per
  `Sender` address.
- The <i>Dust Deposit Unlock Condition</i>, and hence the output and transaction that contain it, is valid, if and only
  if an output with the `Sender` address is consumed and unlocked in the transaction. If `Sender` is either
...
<details>
<summary>Dust Deposit Return Unlock Condition</summary>
<blockquote>
...
<table>
    ...
    <tr>
        ...
        <td>uint8</td>
        <td>
            Set to <strong>value 1</strong> to denote a <i>Dust Deposit Return Unlock Condition</i>.
        </td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>uint64</td>
        <td>
            Amount of IOTA tokens the consuming transaction should deposit to the address defined in <i>Sender</i>.
        </td>
    </tr>
</table>
...
together with the [Expiration Unlock Conditions](#expiration-blocks). The receiving party then has a sender-defined
...
###### Additional semantic transaction validation rules:
- An output that has <i>Timelock Milestone Index Unlock Condition</i> specified must only be consumed and unlocked in a
  transaction, if the confirming milestone index is ≥ than the `Milestone Index` specified in the unlock condition.
- An output that has <i>Timelock Unix Unlock Condition</i> specified must only be consumed and unlocked in a
  transaction, if the timestamp of the confirming milestone is equal or past the `Unix Time` specified in the unlock
  condition.
- An output that has both <i>Timelock Milestone Index Unlock Condition</i> and  <i>Timelock Unix Unlock Condition</i>
  specified must only be consumed and unlocked in a transaction if both unlock conditions validate.

<details>
<summary>Timelock Milestone Index Unlock Condition</summary>
<blockquote>
...
<details>
<summary>Timelock Unix Unlock Condition</summary>
<blockquote>
...
<table>
    ...
    <tr>
        ...
        <td>uint8</td>
        <td>
            Set to <strong>value 3</strong> to denote a <i>Timelock Unix Unlock Condition</i>.
        </td>
    </tr>
    <tr>
        <td>Unix Time</td>
...
has been passed. The expiration might be specified as a unix timestamp or as a milestone index. When specified in both
ways, both conditions have to pass in order for the unlock to be valid.
...
by their senders. Not to mention the possibility to time requests by specifying both a timelock and an expiration
unlock condition.

###### Additional semantic transaction validation rules:

- An output that has <i>Expiration Milestone Index Unlock Condition</i> set must only be consumed and unlocked by the
  target `Address` (defined in <i>Address Unlock Condition</i>) in a transaction that has a confirming milestone index
  < than the `Milestone Index` defined in the unlock condition.
- An output that has <i>Expiration Milestone Index Unlock Condition</i> set must only be consumed and unlocked by the
  `Sender` address in a transaction that has a confirming milestone index ≥ than the `Milestone Index` defined in the
  unlock condition.
- An output that has <i>Expiration Unix Unlock Condition</i> set must only be consumed and unlocked by the target
  `Address` (defined in <i>Address Unlock Condition</i>) in a transaction that has a confirming milestone timestamp
  earlier than the `Unix Time` defined in the unlock condition.
- An output that has <i>Expiration Unix Unlock Condition</i> set must only be consumed and unlocked by the `Sender`
  address in a transaction that has a confirming milestone timestamp same or later than the `Unix Time` defined in the
  unlock condition.
- Semantic validation of an output that has either <i>Expiration Milestone Index Unlock Condition</i> or
  <i>Expiration Unix Unlock Condition</i> set and is unlocked by the `Sender` address must ignore:
  - [Semantic validation of <i>Dust Deposit Return Unlock Condition</i>](#dust-deposit-return-unlock-condition) if present.
- The <i>Expiration Unlock Condition</i>, and hence the output and transaction that contain it, is valid, if and only
  if an output with the `Sender` address is consumed and unlocked in the transaction. If `Sender` is either
  <i>Alias Address</i> or <i>NFT Address</i>, their corresponding outputs (defined by `Alias ID` and `NFT ID`) must be
  unlocked in the transaction.

<details>
<summary>Expiration Milestone Index Unlock Condition</summary>
<blockquote>
    Defines a milestone index until which only <i>Address</i>, defined in <i>Address Unlock Condition</i>, is allowed to
    unlock the output. After the milestone index, only <i>Sender</i> can unlock it.
</blockquote>
</details>
...
<table>
    ...
    <tr>
        ...
        <td>uint8</td>
        <td>
            Set to <strong>value 4</strong> to denote a <i>Expiration Milestone Index Unlock Condition</i>.
        </td>
    </tr>
    <tr>
        <td valign="top">Sender <code>oneOf</code></td>
    ...
    <tr>
        ...
        <td>uint32</td>
        <td>
            Before this milestone index, <i>Address Unlock Condition</i> is allowed to unlock the output, after that only the address defined in <i>Sender</i>.
        </td>
    </tr>
</table>
...
<blockquote>
    Defines a unix time until which only <i>Address</i>, defined in <i>Address Unlock Condition</i>, is allowed to unlock the output. After the expiration time, only <i>Sender</i> can unlock it.
</blockquote>
</details>
...
<table>
    ...
    <tr>
        ...
        <td>uint8</td>
        <td>
            Set to <strong>value 5</strong> to denote a <i>Expiration Unix Unlock Condition</i>.
        </td>
    </tr>
    <tr>
        <td valign="top">Sender <code>oneOf</code></td>
    ...
    <tr>
        ...
        <td>uint32</td>
        <td>
            Before this unix time, <i>Address Unlock Condition</i> is allowed to unlock the output, after that only the address defined in <i>Sender</i>.
        </td>
    </tr>
</table>
...
<table>
    ...
    <tr>
        ...
        <td>uint8</td>
        <td>
            Set to <strong>value 6</strong> to denote an <i>State Controller Address Unlock Condition</i>.
        </td>
    </tr>
    <tr>
        <td>Address</td>
...
<table>
    ...
    <tr>
        ...
        <td>uint8</td>
        <td>
            Set to <strong>value 7</strong> to denote an <i>Governor Address Unlock Condition</i>.
        </td>
    </tr>
    <tr>
        <td>Address</td>
...
increases the required dust deposit. ISC is a great example of a higher layer protocol that makes use of
...
only by their address, but also based on the the `Tag`. **The combination of an <i>Tag Block</i>, a
...
<i>ExtendedOutput</i> can hold native tokens and might have several unlock conditions and optional feature blocks. The
...
Note, that an <i>ExtendedOutput</i> in its simplest possible form with only an <i>Address Unlock Condition</i> and
            ...
            <tr>
                ...
                <td colspan="2">
                    ...
                    <details>
                        <summary>Expiration Milestone Index Unlock Condition</summary>
                        <blockquote>
                            Defines a milestone index until which only <i>Address Unlock Condition</i> is allowed to unlock the output. After the milestone index, only <i>Sender</i> can unlock it.
                        </blockquote>
                        <table>
                            <tr>
                                <td><b>Name</b></td>
                    ...
                    <details>
                        <summary>Expiration Unix Unlock Condition</summary>
                        <blockquote>
                            Defines a unix time until which only <i>Address Unlock Condition</i> is allowed to unlock the output. After the expiration time, only <i>Sender</i> can unlock it.
                        </blockquote>
                        <table>
                            <tr>
                                <td><b>Name</b></td>
...
- Each <i>Native Token</i> must be unique in the set of `Native Tokens` based on its `Token ID`. No duplicates are
  allowed.
- It must hold true that `0` ≤ `Unlock Conditions Count` ≤ `6`.
- `Unlock Condition Type` of an <i>Unlock Condition</i> must define on of the following types:
  - <i>Address Unlock Condition</i>
  - <i>Dust Deposit Return Unlock Condition</i>
  - <i>Timelock Milestone Index Unlock Condition</i>
  - <i>Timelock Unix Unlock Condition</i>
  - <i>Expiration Milestone Index Unlock Condition</i>
  - <i>Expiration Unix Unlock Condition</i>
...
- Governance transition:
    ...
    - Governance transition must only change the following fields:
      `Address` of <i>State Controller Address Unlock Condition</i>,`Address` of <i>Governor Address Unlock Condition</i>,
...
- Indexers and nodeplugins shall map the alias address of the output derived with `Alias ID` to the regular
...
- `Unlock Condition Type` of an <i>Unlock Condition</i> must define on of the following types:
  - <i>Address Unlock Condition</i>
- <i>Address Unlock Condition</i> might only define an <i>Alias Address</i> as `Address`.
...
- When neither `Current State` nor `Next State` is empty:
    ...
    -  `Address` field of <i>Address Unlock Condition</i> must not change.
...
- The `Foundry ID` of a foundry output should have a global mapping table in nodes, so that given a `Foundry ID`, the
  ...
  While it is not necessarily needed for the protocol, it is needed for client side operations (what is the current
...
The NFT may contain immutable metadata set upon creation, and a verified `Issuer`. The output type supports all
 unlock conditions and optional feature blocks so that the output can be sent as a request to smart contract chain
(alias) accounts.
...
- Syntactic validation of all present unlock conditions must pass.
- <i>Address Unlock Condition</i> must be present.
- It must hold true that `0` ≤ `Blocks Count` ≤ `4`.
- `Block Type` of a <i>Block</i> must define on of the following types:
  - <i>Sender Block</i>
  - <i>Issuer Block</i>
  - <i>Metadata Block</i>
