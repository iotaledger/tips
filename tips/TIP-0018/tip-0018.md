...
- An output that has <i>Storage Deposit Return Unlock Condition</i> specified must only be consumed and unlocked in a
  transaction that deposits `Return Amount` IOTA coins to `Return Address` via one or more outputs that:
   ...
   - have no [<i>Feature Blocks</i>](#feature-blocks).
- When several outputs with <i>Storage Deposit Return Unlock Condition</i> and the same `Return Address` are consumed,
  ...
  total sum per `Return Address` via output(s) that satisfy the previous condition.
...
###### Additional syntactic transaction validation rules:
- An output with <i>Metadata Block</i> is valid, if and only if 0 < `length(Data)` ≤ `Max Metadata Length`.
...
##### Additional syntactic transaction validation rules:
- An output with <i>Tag Block</i> is valid, if and only if 0 < `length(Tag)` ≤
  `Max Tag Length`.
...
- `Token Tag` part of the `Token ID` (last 12 bytes) matches the `Token Tag` field of the foundry output.
- When neither `Current State` nor `Next State` is empty:
