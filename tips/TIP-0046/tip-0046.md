...
- Time restrictions for blocks including a transaction:
  - If a `Block` contains a transaction payload. The Slot Index from `Issuing Time` of a block must be greater or equal
    than Issuing Time slot of the transaction included.
