...
- If a `Block` contains a transaction payload, the following conditions must be checked.
  ...
  - If the transaction includes a `Commitment Input`, that input's `SlotIndex` field must be in the closed interval `[BlockSlot-MaxCommittableAge, BlockSlot-MinCommittableAge]`
