...
`Unix Timestamp` is `(Unix Timestamp - Genesis Unix Timestamp)/Slot Duration In Seconds + 1`. The `+ 1` is required
because slots are counted starting from `1` with `0` being reserved for the genesis, which has to be addressable as its
own slot.
...
index of a slot index `Slot Index` is `0` if `Slot Index == 0`, otherwise it is
`(Slot Index >> Slots Per Epoch Exponent) + 1`, where `>>` is the _zero-fill right-shift_ or _logical shift_ operation.
The `+ 1` is required because epochs are counted starting from `1` with `0` being reserved for the genesis, which has to
be addressable as its own epoch.
