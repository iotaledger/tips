...
- If a `Block` contains a transaction payload. The Slot Index corresponding to the `Issuing Time` of a block must be
  greater or equal than the `Creation Slot` of the contained transaction.
