...
author: Levente Pap <levente.pap@iota.org>
...
| Active | The TIP is deployed to the main network or some TIP specific addoption criteria has been met. |
...
A `Draft` TIP might be moved to `Rejected` status by _TIP editor(s)_ or _Technical Committee_ if it does not meet the appropriate TIP criterias, or no relevant progress has been demonstrated on the TIP for at least 3 years.
...
Once the TIP is ready to be merged as a draft, the ditor will:
...
## Backwards Compatibility
 - The current `iotaledger/protocol-rfcs` repository will be renamed to `iotaledger/tip`.
