...
- It must hold that: `1 <= Addresses Count <= 10`.
- The addresses must be lexically ordered and unique based on the contained serialized address.
- It must hold for the `Weight` of a contained address: `Weight >= 1`.
- It must hold for the cumulative weight `Cumulative Weight` of all `Weight`s in `Addresses`:
  `Cumulative Weight >= Threshold`.
- It must hold that: `Threshold >= 1`.
...
- It must hold that: `1 <= Unlocks Count <= 10`.
...
- It must hold that: `Cumulative Unlocked Weight >= Threshold`.
...
- One alternative would be to have two sets of addresses, where one lists the mandatory and one the optional addresses,
  the latter of which contains weights and a threshold. While this allows for similiar use cases, it is harder to
...
The resulting Bech32 representation should be:
