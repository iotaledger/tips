...
The  _Transaction Essence_ itself can contain another payload as described in general in  [TIP-24](../TIP-0024/tip-004.md). The [semantic validity](#semantic-validation) of the encapsulating _Transaction Payload_ does not have any impact on the payload.
...
* The count of all distinct native tokens present in the UTXOs referenced by inputs and in the transaction outputs must not be larger than `Max Native Token Count`. A native token that occurs several times in inputs and outputs is counted as one.
