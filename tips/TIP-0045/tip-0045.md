...
- $\text{IOTA}_i*f(t - t_i)$ is the amount of potential Mana generated by consuming output `i` ($\text{IOTA}_i$ is the amount of tokens in output `i`, and $f$ is given by the lookup table 1: Potential Mana generation in [TIP-39](../TIP-0039/tip-0039.md));
- $\text{Mana}_i * g(t - t_i)$ is the amount of stored Mana on the consuming output `i` with applied decay ($\text{Mana}_i$ is the amount of stored Mana in output `i`, and $g$ is given by the lookup table 2: decays for Mana in [TIP-39](../TIP-0039/tip-0039.md));
