  ...
  <tr>
    ...
    <td colspan="2">
      <details open="true">
        ...
        <blockquote>A transaction. Defined in <a href='../TIP-0045/tip-0045.md#transaction'>TIP-45 (Transaction)</a>.</blockquote>
        <table>
          <tr>
            <td>
              <b>Name</b>
...
The _Transaction ID_ is a `ByteArray[36]` which is computed as follows:
- Let `Essence Bytes` be the concatenated serialization of the fields from `Network ID` to `Payload`.
- Let `Output Bytes` be the concatenated serialization of the fields `Outputs Count` and `Outputs`.
- Let `ID` be the BLAKE2b-256 of the concatenation of `Essence Bytes` and `Output Bytes`.
