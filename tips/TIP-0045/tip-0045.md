...
identified by `Account ID` at the `Slot Index` given by the _Commitment Input_. Multiple such inputs can be present in a
single transaction to provide the BIC value for different accounts; however, no two _Block Issuance Credits Inputs_ can
...
- Essence:
  ...
  - Outputs:
    ...
    - The sum of all `Mana` fields must not exceed `2^Mana Bits Exponent - 1`.
