...
Transaction Capabilities are represented as a `ByteArray` with a length prefix. For the transaction that contains them to be
...
This list is an Allowlist: If the bit is `1` (**set**) the transaction has the capability, if the bit is `0` (**unset**) it
does not have it.
...
| 3          | Can destroy Foundry Outputs. |
| 4          | Can destroy NFT Outputs.     |
